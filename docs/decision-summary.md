...
| DEC-011 | Mixed-sample generation mechanism (Provisional — redesigned, confirmation round found a category-specific split) | Three ground-truth regimes: surgical sentence/paragraph splice (A/B) vs. whole-essay (C, essay-level-only). **Confirmation (n=10, 50 records): paragraph-level (B) close to validated (19/20 QC-passed, 18/20 meaning-preserved); sentence-level (A) NOT ready — 47% of reviewed samples changed meaning despite passing structural QC** | Sequence-alignment (`difflib`) to rescue whole-essay diffing into sentence labels | Structural QC (length/resegmentation) does not catch semantic drift — 4 samples passed every automated check while changing meaning, which is why `semantic_preservation` (manual-review-only field) was added and why sentence-level is recommendation "B: promising but needs another revision," not scaled |
