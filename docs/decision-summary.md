...
| DEC-011 | Mixed-sample generation mechanism (Provisional — partially invalidated) | Surgical splice validated (6/10, 9/10 passed); whole-essay-diff for polish **failed 70% of the time** (structure drift) — needs sequence-alignment redesign, not a threshold tune | One uniform whole-essay-diff mechanism for all categories | EXP-DATA-001 found position-based diffing breaks when the model consolidates sentences despite instructions not to; exact-count-match alignment discards most attempts instead of recovering partial ground truth |
